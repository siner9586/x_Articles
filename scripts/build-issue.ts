...
import { listFiles, loadYamlList, readJson, readText, writeJson } from './utils/fs.js';
...
async function previousIssueDates() {
  const files = await listFiles('data/issues', '.json');
  return files.map(f => f.match(/(\d{4}-\d{2}-\d{2})\.json$/)?.[1]).filter(Boolean) as string[];
}
...
async function run() {
  ...
  const issue = {
    ...
    summary: {
      one_liner: isInitialSourceIndex
        ...
        : `本期从 ${candidates.length} 条候选中筛出 ${selectedCount} 条公开 Articles，默认按质量而非热度排序。`,
      main_trends: isInitialSourceIndex
        ? ['来源库已覆盖 AI 公司、应用、Infra、VC、媒体、研究机构与社区发现入口。', '首次运行没有用 mock 文章冒充真实抓取结果。']
        : Array.from(new Set(selected.flatMap(i => i.topics))).slice(0, 6),
      what_to_watch: isInitialSourceIndex
        ? ['下一期自动运行将继续从 RSS、公开博客、人工 curated 链接中寻找新增 Articles。', 'X 仅作为发现入口；无法合规访问的内容会记录 fetch_status，而不会强抓正文。']
  ...
  console.log(`Built issue ${issueDate}: ${selectedCount} selected, ${blockedDuplicates.length} duplicate candidates blocked.`);
}
